...
contains
  ...
  subroutine killing_geodesic &
    ...
    dtheta = killing_interp (qlm_xi_t(:,:,hn), &
         org_theta, org_phi, del_theta, del_phi, theta, phi_an, ierr1)
    dphi   = killing_interp (qlm_xi_p(:,:,hn), &
         org_theta, org_phi, del_theta, del_phi, theta, phi_an, ierr2)
    ...
    dlambda = (phi1 - phi_an) / dphi

    nsteps = nsteps + 1
    lmbda = lmbda + dlambda
    theta  = theta  + dlambda * dtheta
    phi_an    = phi_an    + dlambda * dphi
