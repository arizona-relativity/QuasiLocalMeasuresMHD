...
subroutine qlm_calc_tetrad1 (CCTK_ARGUMENTS, hn)
  ...
  CCTK_REAL    :: mm1(0:3), mm2(0:3) ! temporary variables to calculate mm
  ...
  CCTK_COMPLEX :: mmm(0:3)          ! vector on horizon within the hypersurface
     ...
     do i = 1+qlm_nghoststheta(hn), qlm_ntheta(hn)-qlm_nghoststheta(hn)
        ...
        do aa=1,3
           do b=1,3
              do c=1,3
                 dee(aa,b,c) = 0
                 do d=1,3
                    dee(aa,b,c) = dee(aa,b,c) + ee_inv(c,d) * dee_spher(aa,b,d)
                 end do
              end do
           end do
        end do
        ...
        do aa=0,3
           do b=0,3
              gee(:,aa,b) = dee(:,aa,b)
              do c=0,3
                 gee(:,aa,b) = gee(:,aa,b) + ee(:,c) * gamma4(aa,c,b)
              end do
           end do
        end do
        ...
        mm1(:) = ee(3,:)
        gm1(:,:) = gee(3,:,:)
        call gram_schmidt_normalise (g4,gg4, mm1,gm1, one)

        ! m2 = et
        mm2(:) = ee(2,:)
        gm2(:,:) = gee(2,:,:)
        call gram_schmidt_project (g4,gg4, mm1,gm1, one, mm2,gm2)
        call gram_schmidt_normalise (g4,gg4, mm2,gm2, one)

        ! ss = er
        ss(:) = ee(1,:)
        gss(:,:) = gee(1,:,:)
        call gram_schmidt_project (g4,gg4, mm1,gm1, one, ss,gss)
        call gram_schmidt_project (g4,gg4, mm2,gm2, one, ss,gss)
        ...
        mmm = cmplx(mm1, mm2, kind(mmm)) / sqrt(two)
        ...
        do aa=0,3
           do b=0,3
              ...
              nabla_mm(aa,b) = 0
              do c=0,3
                 nabla_ll(aa,b) = nabla_ll(aa,b) + g4(aa,c) * gll(c,b)
                 nabla_nn(aa,b) = nabla_nn(aa,b) + g4(aa,c) * gnn(c,b)
                 nabla_mm(aa,b) = nabla_mm(aa,b) + g4(aa,c) * gmm(c,b)
              end do
              qlm_tetrad_derivs(i,j)%nabla_ll(aa,b) = nabla_ll(aa,b)
              qlm_tetrad_derivs(i,j)%nabla_nn(aa,b) = nabla_nn(aa,b)
              qlm_tetrad_derivs(i,j)%nabla_mm(aa,b) = nabla_mm(aa,b)
           end do
        end do
