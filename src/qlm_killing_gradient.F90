...
subroutine qlm_killing_gradient (CCTK_ARGUMENTS, hn)
  ...
  do j = 1+qlm_nghostsphi(hn), qlm_nphi(hn)-qlm_nghostsphi(hn)
     do i = 1+qlm_nghoststheta(hn), qlm_ntheta(hn)-qlm_nghoststheta(hn)
        ...
        do aa=1,2
           do b=1,2
              ndpsi2 = ndpsi2 + qu(aa,b) * dpsi2(aa) * dpsi2(b)
           end do
        end do
        ndpsi2 = sqrt(ndpsi2)
        ...
        do aa=1,2
           ...
           do b=1,2
              do c=1,2
                 dndpsi2(aa) = dndpsi2(aa) + 1 / (2*ndpsi2) * (qu(b,c) * ddpsi2(b,aa) * dpsi2(c) + qu(b,c) * dpsi2(b) * ddpsi2(c,aa) + dqu(b,c,aa) * dpsi2(b) * dpsi2(c))
              end do
           end do
        end do
#endif
        ...
        do aa=1,2
           xi(aa) = 0
           do b=1,2
              xi(aa) = xi(aa) + sqrt(dtq) * epsilon2(aa,b) * dpsi2(b)
           end do
        end do
        ...
        do aa=1,2
           xi(aa) = 0
           do b=1,2
              xi(aa) = xi(aa) + sqrt(dtq) * epsilon2(aa,b) * dndpsi2(b)
           end do
        end do
  ...
  do j = 1+qlm_nghostsphi(hn), qlm_nphi(hn)-qlm_nghostsphi(hn)
     do i = 1+qlm_nghoststheta(hn), qlm_ntheta(hn)-qlm_nghoststheta(hn)
        ...
        do aa=1,2
           do b=1,2
              chi = chi - half * sqrt(dtq) * epsilon2(aa,b) * dxi(b,aa)
           end do
        end do
