...
subroutine qlm_calc_weyl_scalars1 (CCTK_ARGUMENTS, hn)
...
!        qlm_rsc4(i,j,hn) = rsc4
        ...
        do aa=0,3
           do b=0,3
              do c=0,3
                 do d=0,3
                    qlm_psi0(i,j,hn) = qlm_psi0(i,j,hn) + we4(aa,b,c,d) * ll(aa) * mmm(b) * ll(c) * mmm(d)
                    qlm_psi1(i,j,hn) = qlm_psi1(i,j,hn) + we4(aa,b,c,d) * ll(aa) * mmm(b) * ll(c) * nn(d)
                    qlm_psi2(i,j,hn) = qlm_psi2(i,j,hn) + we4(aa,b,c,d) * ll(aa) * mmm(b) * conjg(mmm(c)) * nn(d)
                    qlm_psi3(i,j,hn) = qlm_psi3(i,j,hn) + we4(aa,b,c,d) * ll(aa) * nn(b) * conjg(mmm(c)) * nn(d)
                    qlm_psi4(i,j,hn) = qlm_psi4(i,j,hn) + we4(aa,b,c,d) * conjg(mmm(aa)) * nn(b) * conjg(mmm(c)) * nn(d)
                 end do
              end do
           end do
        end do
        ...
        do aa=0,3
           do b=0,3
              ...
              qlm_phi00(i,j,hn) = qlm_phi00(i,j,hn) - 1/two * ri4(aa,b) * ll(aa) * ll(b)
              qlm_phi11(i,j,hn) = qlm_phi11(i,j,hn) - 1/two * ri4(aa,b) * (ll(aa) * nn(b) + mmm(aa) * conjg(mmm(b))) / 2
              qlm_phi01(i,j,hn) = qlm_phi01(i,j,hn) - 1/two * ri4(aa,b) * ll(aa) * mmm(b)
              qlm_phi12(i,j,hn) = qlm_phi12(i,j,hn) - 1/two * ri4(aa,b) * nn(aa) * mmm(b)
              qlm_phi10(i,j,hn) = qlm_phi10(i,j,hn) - 1/two * ri4(aa,b) * ll(aa) * conjg(mmm(b))
              qlm_phi21(i,j,hn) = qlm_phi21(i,j,hn) - 1/two * ri4(aa,b) * nn(aa) * conjg(mmm(b))
              qlm_phi02(i,j,hn) = qlm_phi02(i,j,hn) - 1/two * ri4(aa,b) * mmm(aa) * mmm(b)
              qlm_phi22(i,j,hn) = qlm_phi22(i,j,hn) - 1/two * ri4(aa,b) * nn(aa) * nn(b)
              qlm_phi20(i,j,hn) = qlm_phi20(i,j,hn) - 1/two * ri4(aa,b) * conjg(mmm(aa)) * conjg(mmm(b))
        ...
        do aa=1,3
           do b=1,3
              trkAH = trkAH + (gu(aa,b) - ss(aa) * ss(b)) * nabla_ss(aa,b)
           end do
        end do
        ...
        do aa=1,3
           do b=1,3
              do c=1,3
                 do d=1,3
                    kk_kk = kk_kk + &
                         (gu(aa,c) - ss(aa) * ss(c)) * nabla_ss(aa,b) * &
...
end subroutine qlm_calc_weyl_scalars1
