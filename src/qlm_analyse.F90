...
subroutine qlm_analyse (CCTK_ARGUMENTS, hn)
  ...
  CCTK_REAL    :: dtg, gu(3,3), trk_loc, dgu(3,3,3)
  ...
  CCTK_REAL    :: det_Jxy, det_Jxz, det_Jyz
  ...
  if (veryverbose/=0) then
     call CCTK_INFO ("Calculating spin and Bflux")
  end if
  ...
  ! Polar circumference at phi_an=0
  j_p0 = 1+qlm_nghostsphi(hn)

  ! Polar circumference at phi_an=pi/2
  ...
  do j = 1+qlm_nghostsphi(hn), qlm_nphi(hn)-qlm_nghostsphi(hn)
     do i = 1+qlm_nghoststheta(hn), qlm_ntheta(hn)-qlm_nghoststheta(hn)
        ...
        ! retrieve Jacobian of horizon surface
        det_Jxy = qlm_det_Jxy(i,j,hn)
        det_Jxz = qlm_det_Jxz(i,j,hn)
        det_Jyz = qlm_det_Jyz(i,j,hn)

        call calc_det (gg, dtg)
        call calc_inv (gg, dtg, gu)
        call calc_trace (gu, kk, trk_loc)
        ...
        do aa=1,3
           do b=1,3
              npspin = npspin + 2 * real((npalpha + conjg(npbeta)) * xi1(aa) * gg(aa,b) * mmm(b))
           end do
        end do
        ...
        ! just to test the diagnostic, let's set sqrt(gamma)Bz = const, such that the determinant does not factor in the integral
        ! and By=Bx=0
        !B_x = 0
        !B_y = 0
        !B_z = 1/qlm_spatial_vol_in_qlm(i,j) 

        ! calculate Bflux in this horizon element
        Bflux = ABS(B_z * det_Jxy + B_x * det_Jyz - B_y * det_Jxz)

        ! multiplying by the area element
        Bflux = Bflux * qlm_spatial_vol_in_qlm(i,j)

        qlm_Bflux(hn) = qlm_Bflux(hn) + Bflux * weights(i)

        ! x = sin theta cos phi_an
        ! y = sin theta sin phi_an
        ! z = cos theta
        ! xi_x = (0,-z,y)
        ! xi_y = (z,0,-x)
        ! xi_z = (-y,x,0)
        ...
        do aa=1,3
           do b=1,3
              coordspinx = coordspinx + xi1_x(aa) * ss(b) * kk(aa,b)
              coordspiny = coordspiny + xi1_y(aa) * ss(b) * kk(aa,b)
              coordspinz = coordspinz + xi1_z(aa) * ss(b) * kk(aa,b)
           end do
        end do
        ...
        do aa=1,3
           do b=1,3
              do c=1,3
                 adm_energy = adm_energy + (delta3(aa,b) * dh4(aa,c,b) &
                      & - gu(aa,b) * dh4(aa,b,c) - h4(aa,b) * dgu(aa,b,c) ) * ss(c)
              end do
           end do
        end do
        ...
        do aa=1,3
           ...
           do b=1,3
              do c=1,3
                 adm_mom(aa) = adm_mom(aa) &
                      &        + gu(aa,c) * kk(c,b) * ss(b)
              end do
              adm_mom(aa) = adm_mom(aa) &
                   + delta3(aa,b) * trk_loc * ss(b)
           end do
        end do
        ...
        do aa=1,3
           ...
           do b=1,3
              do c=1,3
                 do l=1,3
                    do m=1,3
                       adm_amom(aa) = adm_amom(aa)  &
                            &         + epsilon3(aa,b,c) * xx(b) * gu(c,m) * kk(m,l) * ss(l)
                    end do
                    adm_amom(aa) = adm_amom(aa)  &
                         &    - epsilon3(aa,b,c) * xx(b) *  delta3(c,l) * trk_loc * ss(l)
                 end do
              end do
           end do
        end do
        ...
        do aa=1,3
           do b=1,3
              w_energy = w_energy + (dh4(b,b,aa) - dh4(aa,b,b)) * ss(aa)
           end do
        end do
        ...
        do aa=1,3
           ...
           do b=1,3
              w_mom(aa) = w_mom(aa) &
                   + (- dh4(b,b,0) + dh4(b,0,b)) * ss(aa) &
                   + (- dh4(aa,0,b) + dh4(b,aa,0)) * ss(b)
           end do
        end do
        ...
        do aa=1,3
           do b=1,3
              w_amom(aa,b) = 0
              do c=1,3
                 w_amom(aa,b) = w_amom(aa,b) + ss(c) * ( &
                      + (- xx(aa) * dh4(0,b,c) + xx(b) * dh4(0,aa,c)) &
                      + (+ xx(aa) * dh4(b,c,0) - xx(b) * dh4(aa,c,0)) &
                      + (h4(0,b) * delta4(c,aa) - h4(0,aa) * delta4(c,b)))
              end do
           end do
        end do
  ...
  qlm_Bflux(hn)  = qlm_Bflux(hn) / 2  ! divide by 2 otherwise double counting flux through horizon
  ...
  ! This is wrong when there is charge
  ...
  if (verbose/=0) then
     ...
     call CCTK_INFO (msg)
     write (msg, '("   Polar circumference at phi_an=0:    ",g16.6)') &
          qlm_polar_circumference_0(hn)
     call CCTK_INFO (msg)
     write (msg, '("   Polar circumference at phi_an=pi/2: ",g16.6)') &
     ...
     call CCTK_INFO (msg)
     write (msg, '("   Horizon B-flux:                               ",g14.6)') qlm_Bflux(hn)
     call CCTK_INFO (msg)
